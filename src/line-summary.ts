...
export async function buildLineSummaries() {
  ...
  while (true) {
    ...
    for (const route of routes) {
      ...
      processed++
      const displayName = route.shortName || route.longName || route.routeId
      reportImportActivity(`Optimisation de la ligne ${displayName}`, {
        phase: 'summarizing',
        phasePercent: total ? (processed / total) * 100 : 100,
        detail: `Calcul des horaires et du parcours représentatif — ${processed}/${total}`,
        currentItem: displayName,
        processed,
        total,
        unit: 'lignes',
        counters: { summarizedRoutes: processed },
      })
