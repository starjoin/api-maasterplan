...
La section **API externes et sources de données** réunit RFU GTFS, RFU NeTEx, Navitia et SIRI VM, avec leurs adresses publiques, fréquence, configuration d’accès et données disponibles. Navitia présente un bilan par base publiée et deux boutons permettent d’actualiser séparément les tracés GTFS ou NeTEx. Cette actualisation crée un instantané SQLite cohérent de la base concernée, ne télécharge que Navitia, valide la copie, puis la publie atomiquement ; les fichiers sources sont conservés par liens physiques. SIRI VM affiche les véhicules en cache, son âge et la dernière erreur. Les accès configurés ne constituent pas un test de connexion. Le dashboard lit les métadonnées SQLite et le cache SIRI, sans appeler les fournisseurs. Le registre `src/admin/integrations.ts` et un composant générique permettent d’ajouter d’autres API. Les échecs Navitia des prochains imports sont enregistrés dans leurs statistiques ; les anciennes bases peuvent nécessiter un réimport pour obtenir ce bilan.
